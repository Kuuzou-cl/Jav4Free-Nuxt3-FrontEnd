...
const { data: dataScene } = await useFetch('https://jav.souzou.dev/scenes/sceneIdv2?id=' + id);

const videoPreviewMini = ref(0);

let newSceneTitle = dataScene._rawValue.Scene.title;
let newSceneCode = dataScene._rawValue.Scene.code;
let newSceneDuration = dataScene._rawValue.Scene.duration;
let newSceneHide = dataScene._rawValue.Scene.hide;

let videoPreview = dataScene._rawValue.Scene.video480p;
console.log(videoPreviewMini._rawValue.src);
...
newSceneCategories._rawValue.forEach(element => {
    if (dataScene._rawValue.Categories.find((objCategory) => objCategory.id === element.id)) {
...
const changeUrlVideo = async () => {
    videoPreview = "https://d27vxor1f495av.cloudfront.net/scenes_480/" + newSceneCode + "_1.mp4";
...
const postScene = async () => {
    ...
    const { data, error } = await useFetch('https://jav.souzou.dev/scenes/updateScenev2', {
        method: 'PATCH',
        headers: myHeaders,
        body: {
            id: id,
